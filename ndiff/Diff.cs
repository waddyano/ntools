...
namespace qed
{
    ...
    public class Diff
    {
        ...
        public struct Item
        {
            ...
            {
                return "[StartA=" + StartA + ",StartB=" + StartB + ",DeletedA=" + DeletedA + ",InsertedB=" + InsertedB + "]";
            }
        } // Item
        ...
        public static string TestHelper(Item[] f)
        {
            ...
            Trace.WriteLine("diff " + ret.ToString());
            return (ret.ToString());
        }
#endif
        #endregion
        ...
        {
            LineCollection l1 = new LineCollection();
            l1.Open(args[0]);
            LineCollection l2 = new LineCollection();
            l2.Open(args[1]);

            Item[] diffs = DiffText(l1, l2, null);
            if (true)
            {
                int context = 3;
                for (int i = 0; i < diffs.Length; ++i)
                {
                    ...
                    Console.WriteLine("@@ -{0},{1} +{2},{3} @@", diffs[i].StartA + 1, 99, diffs[i].StartB, 99);
                    int firstContext = diffs[i].StartA - context;
                    if (firstContext < 0)
                        firstContext = 0;
                    for (int line = firstContext; line < diffs[i].StartA; ++line)
                    {
                        Console.Write(" ");
                        Console.WriteLine(l1[line].Text);
                    }
                    for (int j = 0; j < diffs[i].DeletedA; ++j)
                    {
                        Console.Write("-");
                        Console.WriteLine(l1[diffs[i].StartA + j].Text);
                    }
                    for (int j = 0; j < diffs[i].InsertedB; ++j)
                    {
                        Console.Write("+");
                        Console.WriteLine(l2[diffs[i].StartB + j].Text);
                    }
                }
            }
            else
        ...
        private static Item[] CreateDiffs(DiffData DataA, DiffData DataB)
        {
            ...
            return (Item [])a.ToArray(typeof(Item));
        }
